...
fn err(ctor: fn(String) -> FetchError) -> impl Fn(JsValue) -> FetchError {
    move |error: JsValue| ctor(error.as_string().unwrap_or_else(|| "unknown".to_string()))
}
...
pub async fn fetch(path: &str) -> Result<Response, FetchError> {
    ...
    let request =
        Request::new_with_str_and_init(path, &opts).map_err(err(FetchError::NewRequest))?;
...
pub async fn fetch_array_buffer(path: &str) -> Result<ArrayBuffer, FetchError> {
    ...
    let array_buffer: ArrayBuffer = {
        let promise = response
            .array_buffer()
            .map_err(err(FetchError::GetResult))?;
        let value = JsFuture::from(promise)
            .await
            .map_err(err(FetchError::AwaitResult))?;
...
pub async fn fetch_blob(path: &str) -> Result<Blob, FetchError> {
    ...
    let blob: Blob = {
        let promise = response.blob().map_err(err(FetchError::GetResult))?;
        let value = JsFuture::from(promise)
            .await
            .map_err(err(FetchError::AwaitResult))?;
