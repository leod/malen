            ...
            case 1:
                f_color = vec4(
                    textureLod(
                        screen_albedo,
                        v_tex_coords,
                        float(global_light_props.debug_mipmap)
                    ).rgb,
                    1.0
                );
                break;
...
const CONE_TRACING_SOURCE: Glsl = glsl! {
    vec3 trace_cone(vec2 origin, vec2 dir) {
        ...
        for (int i = 0; i < {{num_tracing_steps}} && occlusion <= 1.0; i += 1) {
            ...
            t += min(
                global_light_props.indirect_step_factor * cone_diameter,
                global_light_props.indirect_max_step
            );
        }
